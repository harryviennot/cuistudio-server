...
def apply_migration(migration_file: str):
    ...
    print(f"   • Navigate to SQL Editor")
    print(f"   • Copy and paste the contents from:")
    print(f"     {migration_path}")
    print(f"   • Click 'Run'\n")

    print("2️⃣  Using psql (PostgreSQL client):")
    print(f"   psql 'postgresql://postgres:[PASSWORD]@[HOST]/postgres' \\")
    ...
    print(f"   {migration_path}\n")
    print(f"📝 What this migration does:")
    print(f"   • Creates helper functions to migrate instruction format")
    print(f"   • Updates existing recipes from old format (text field)")
    print(f"     to new format (title + description fields)")
    print(f"   • Adds validation functions for future use")
    print(f"   • Preserves all existing data during migration")
    print(f"{'='*80}\n")
